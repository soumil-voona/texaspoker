...
if not st.session_state.submitted:
   ...
   if st.session_state.playing:
       ...
       money1 = st.session_state.money.copy()
       playerNames = st.session_state.playerNames.copy()
       newly_eliminated = []
       remaining_money = []
       remaining_names = []
       ...
       for i in range(len(money1)):
           if money1[i] == 0:
               newly_eliminated.append(playerNames[i])
           else:
               remaining_money.append(money1[i])
               remaining_names.append(playerNames[i])

       eliminatedString = ", ".join(newly_eliminated)
       if newly_eliminated:
           st.warning(f"Players eliminated: {eliminatedString}")
       gameCompleted = False
       if len(remaining_names) == 1:
               st.success(f"Game Over! {remaining_names[0]} wins the tournament!")
               gameCompleted = True
       print(gameCompleted)
       if st.button(("Play Next Game", "Play New Game")[gameCompleted], use_container_width=True):
           if len(remaining_names) == 1:
               st.success(f"Game Over! {remaining_names[0]} wins the tournament!")

               for key in list(st.session_state.keys()): #Stack Overflow
                   del st.session_state[key] #Stack Overflow
               st.rerun()
           st.session_state.money = remaining_money
           st.session_state.playerNames = remaining_names
           st.session_state.deck = Deck()
           st.session_state.deck.shuffle()
           st.session_state.players = []
           st.session_state.pot = 15
           ...
           for i in range(len(remaining_names)):
               st.session_state.players.append(Hand(remaining_names[i]))
               for _ in range(2):
                   st.session_state.players[i].add_card(st.session_state.deck.deal_one())
               st.session_state.player_bets.append(0)
               st.session_state.players_playing.append(True)
           ...
           small_blind = st.session_state.round % len(remaining_names)
           big_blind = (st.session_state.round + 1) % len(remaining_names)
           ...
           st.session_state.players_played[small_blind] = True
           st.session_state.players_played[big_blind] = True
           st.session_state.currentPlayer = (st.session_state.round + 2) % len(remaining_names)
           st.rerun()