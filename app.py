...
def check_winner(cards):
	...
	# Return High card if nothing else
...
if not st.session_state.submitted:
	...
	if st.session_state.playing:
        ...
        #Checks if round is complete if everyone played
		def round_complete():
		...
		with col2:
			if st.button("Fold", use_container_width=True):
				...
				if round_complete(): next_round()
				st.rerun()
        #Call or check current bet - do the least
		with col3:
		...
		with col5:
			if st.button("Show Cards", use_container_width=True):
				...
				st.rerun()
    #Run once the game is complete to check for winner
	else:
        ...
        #Session State variable to ensure pot isn't redistributed
		...
		if not st.session_state.pot_distributed:
			...
			#Distribute pot evenly to tied players
			else:
